      ...
      <div className="flex items-center justify-center w-full">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[90rem]">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target="_self" rel="noopener noreferrer" className="fan-card block cursor-pointer">{cardContent}</a>
            ) : (
              <div key={index} className="fan-card">{cardContent}</div>
            );
          })}
        </div>
      </div>
