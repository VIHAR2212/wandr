...
// ─── Glass Button ────────────────────────────────────────────────────────────
function GlassBtn(props: { onClick: () => void; children: React.ReactNode; title?: string; size?: "sm" | "md" }) {
  var sz = props.size === "sm" ? "w-8 h-8 text-sm" : "w-10 h-10 text-base";
  return (
...
export default function TripMap({ trip, isDark = true }: TripMapProps) {
  ...
  var markersRef = useRef<maplibregl.Marker[]>([]);

  var [mapReady, setMapReady] = useState(false);
  var [showGlobeBadge, setShowGlobeBadge] = useState(true);
  ...
  // ─── Memoized data (prevents infinite loops) ─────────────────────────
  var allStops = useMemo(function () { return extractStops(trip); }, [trip]);
  var stopsWithCoords = useMemo(function () {
    return allStops.filter(function (s) { return s.coordinates && s.coordinates.length === 2; });
  }, [allStops]);
  var filteredStops = useMemo(function () {
    return stopsWithCoords.filter(function (s) {
  ...
  }, [stopsWithCoords, activeFilter, selectedDay]);
  var uniqueDays = useMemo(function () {
    return [...new Set(allStops.map(function (s) { return s.day; }).filter(Boolean))].sort() as number[];
  }, [allStops]);
  var stopCountByType = useCallback(function (type: string) {
    return allStops.filter(function (s) { return (s.type || "").toLowerCase() === type; }).length;
  }, [allStops]);

  // ─── GeoJSON builder ──────────────────────────────────────────────────
  var buildGeoJSON = useCallback(function (stops: TripStop[]) {
    var coords = stops.map(function (s) { return s.coordinates as [number, number]; });
  ...
  // ─── Render markers ───────────────────────────────────────────────────
  var renderMarkers = useCallback(function (map: maplibregl.Map, stops: TripStop[]) {
    ...
    stops.forEach(function (stop) {
      ...
      el.style.cssText =
        "position:relative;display:flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;background:" + color + ";border:2.5px solid rgba(255,255,255,0.9);box-shadow:0 3px 14px rgba(0,0,0,0.4),0 0 0 1px rgba(255,255,255,0.08),0 0 20px " + color + "44;font-size:18px;cursor:pointer;transition:transform 0.2s,box-shadow 0.2s;z-index:1;";
      ...
      var descH = stop.description ? '<div style="font-size:12px;color:#444;margin-top:8px;line-height:1.5;">' + stop.description + '</div>' : "";
      var tipsH = stop.tips ? '<div style="font-size:11px;color:#f97316;margin-top:6px;padding:6px 8px;background:#fff7ed;border-radius:8px;">💡 ' + stop.tips + '</div>' : "";
      ...
      var timeH = stop.time ? '<span>⏰ ' + stop.time + '</span>' : "";
      var typeH = stop.type ? '<span>• ' + stop.type + '</span>' : "";
      var html = '<div style="font-family:system-ui,sans-serif;padding:6px 2px 2px;"><div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;"><span style="font-size:16px;">' + emoji + '</span><div style="font-size:14px;font-weight:700;color:#111;">' + (stop.name || "Stop") + '</div></div><div style="font-size:11px;color:#888;display:flex;align-items:center;gap:4px;flex-wrap:wrap;"><span style="background:' + color + '18;color:' + color + ';padding:1px 7px;border-radius:6px;font-weight:600;">Day ' + dayNum + '</span>' + timeH + " " + typeH + '</div>' + descH + tipsH + '<div style="display:flex;gap:12px;margin-top:8px;">' + costH + ratH + '</div></div>';
      var popup = new maplibregl.Popup({ offset: 18, closeButton: true, className: "wandr-popup", maxWidth: "280px" }).setHTML(html);
      var marker = new maplibregl.Marker({ element: el, anchor: "center" }).setLngLat(stop.coordinates as [number, number]).setPopup(popup).addTo(map);
      markersRef.current.push(marker);
    });
  }, []);

  // ─── Render route ─────────────────────────────────────────────────────
  var renderRoute = useCallback(function (map: maplibregl.Map, stops: TripStop[]) {
    var geojson = buildGeoJSON(stops);
    if (!geojson) return;
    ["route-glow", "route-main", "route-dash"].forEach(function (s) { if (map.getSource(s)) map.removeSource(s); });
    ["route-glow-l", "route-main-l", "route-dash-l", "route-dots-l"].forEach(function (id) { if (map.getLayer(id)) map.removeLayer(id); });
    if (map.getSource("route-dots")) map.removeSource("route-dots");
    map.addSource("route-glow", { type: "geojson", data: geojson });
    map.addLayer({ id: "route-glow-l", type: "line", source: "route-glow", paint: { "line-color": "#f97316", "line-width": 14, "line-opacity": 0.12, "line-blur": 10 }, layout: { "line-cap": "round", "line-join": "round" } });
    map.addSource("route-main", { type: "geojson", data: geojson });
    map.addLayer({ id: "route-main-l", type: "line", source: "route-main", paint: { "line-color": "#f97316", "line-width": 3.5, "line-opacity": 0.85 }, layout: { "line-cap": "round", "line-join": "round" } });
    map.addSource("route-dash", { type: "geojson", data: geojson });
    map.addLayer({ id: "route-dash-l", type: "line", source: "route-dash", paint: { "line-color": "#fbbf24", "line-width": 2, "line-opacity": 0.5, "line-dasharray": [0, 4, 3] }, layout: { "line-cap": "round", "line-join": "round" } });
    if (stops.length > 1) {
      var df = stops.map(function (s) { return { type: "Feature" as const, properties: { idx: 0 }, geometry: { type: "Point" as const, coordinates: s.coordinates as [number, number] } }; });
      map.addSource("route-dots", { type: "geojson", data: { type: "FeatureCollection" as const, features: df } });
      map.addLayer({ id: "route-dots-l", type: "circle", source: "route-dots", paint: { "circle-radius": 4.5, "circle-color": "#fff", "circle-stroke-width": 2.5, "circle-stroke-color": "#f97316" } });
    }
  }, [buildGeoJSON]);

  // ─── Fit bounds ───────────────────────────────────────────────────────
  var fitToStops = useCallback(function (map: maplibregl.Map, stops: TripStop[]) {
    if (stops.length === 0) return;
    var b = new maplibregl.LngLatBounds();
    stops.forEach(function (s) { b.extend(s.coordinates as [number, number]); });
    map.fitBounds(b, { padding: { top: 70, bottom: 70, left: 70, right: 70 }, maxZoom: 13, duration: 1200 });
  }, []);
  ...
  useEffect(function () {
    ...
    map.on("error", function (e: any) { console.error("MapLibre:", e.error || e); });
  ...
  // ─── Re-render when filter/day changes (NOT on trip/isDark change) ───
  useEffect(function () {
  ...
  // ─── Toolbar handlers ─────────────────────────────────────────────────
  var zoomIn = function () { if (mapRef.current) mapRef.current.easeTo({ zoom: (mapRef.current.getZoom() || 4) + 1.5, duration: 350 }); };
  var zoomOut = function () { if (mapRef.current) mapRef.current.easeTo({ zoom: Math.max(1, (mapRef.current.getZoom() || 4) - 1.5), duration: 350 }); };
  var resetNorth = function () { if (mapRef.current) mapRef.current.easeTo({ bearing: 0, duration: 400 }); setCompassDeg(0); };
  var toggle3D = function () { if (!mapRef.current) return; mapRef.current.easeTo({ pitch: mapRef.current.getPitch() > 10 ? 0 : 60, duration: 700 }); };
  ...
  var locateMe = function () {
    ...
    navigator.geolocation.getCurrentPosition(function (pos) {
      var lng = pos.coords.longitude; var lat = pos.coords.latitude;
  ...
  var toggleFullscreen = function () { setIsFullscreen(function (p) { return !p; }); setTimeout(function () { if (mapRef.current) mapRef.current.resize(); }, 100); };
  ...
  // ─── Render ────────────────────────────────────────────────────────────
  return (
    <div className={"relative w-full rounded-2xl overflow-hidden transition-all duration-300 " + (isFullscreen ? "fixed inset-4 z-50 rounded-3xl" : "")}
      ...
      {mapReady && (
        <button onClick={function () {} } className="absolute top-3 left-3 z-10 flex items-center gap-2 px-3 py-2 rounded-xl backdrop-blur-md bg-black/30 border border-white/15 text-white/80 text-xs font-medium cursor-default">
      ...
      {mapReady && uniqueDays.length > 1 && (
        <div className="absolute top-14 left-3 z-10 flex flex-wrap gap-1.5 max-w-[200px]">
          <button onClick={function () { setSelectedDay(null); }} className={"px-2.5 py-1 rounded-lg text-[10px] font-bold transition cursor-pointer border " + (selectedDay === null ? "bg-orange-500/80 text-white border-orange-400/40" : "bg-black/20 text-white/50 border-white/10 hover:bg-white/10")}>All</button>
          {uniqueDays.map(function (d) {
            return (<button key={d} onClick={function () { setSelectedDay(selectedDay === d ? null : d); }} className={"px-2.5 py-1 rounded-lg text-[10px] font-bold transition cursor-pointer border " + (selectedDay === d ? "bg-orange-500/80 text-white border-orange-400/40" : "bg-black/20 text-white/50 border-white/10 hover:bg-white/10")}>D{d}</button>);
          })}
        </div>
      )}
      ...
      {mapReady && (
        <div className="absolute top-3 right-3 z-10 flex flex-col gap-1.5">
          ...
          <GlassBtn onClick={toggleTerrain} title="Toggle Terrain">{terrainOn ? "⛰️" : "🏔️"}</GlassBtn>
          <GlassBtn onClick={resetNorth} title="Reset North"><span className="inline-block transition-transform duration-300" style={{ transform: "rotate(" + (-compassDeg) + "deg)" }}>🧭</span></GlassBtn>
          <GlassBtn onClick={locateMe} title="My Location">📍</GlassBtn>
          <GlassBtn onClick={toggle3D} title="Toggle 3D">🗼</GlassBtn>
          <GlassBtn onClick={toggleFullscreen} title="Fullscreen">{isFullscreen ? "⛶" : "⤢"}</GlassBtn>
